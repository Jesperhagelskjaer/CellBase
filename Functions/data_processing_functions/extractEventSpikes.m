...
%     TE = loadcb(cellid,'Events');   
      loadcb(cellid,'Events');   
end

% Load spike times
stimes = loadcb(cellid,'Spikes');
% stimes = double(stimes);
...
for iE = 1:NUMevents
    ...
    if isfield(TE,TriggerEvent1) && isfield(TE,TriggerEvent2)
        fields = fieldnames(TE); %dk
        NUMtrials = length(eval(['TE.',fields{1}])); % dK
...
%              EventTimes2 = TE.(TriggerEvent2) + TE.TrialStart + offset2;
            EventTimes1 = TE.(TriggerEvent1) + offset1; %dk
            EventTimes2 = TE.(TriggerEvent2) +  offset2; %dk
