...
% synchronization
function MakeTrialEvents_FreqDiscrSimpleFreeChoice(sessionpath,AquisitionSyst)
...
for iTSe = 1:SessionData.nTrials
    if iTSe == 1
        TrialStates_FD = [TrialStates_FD SessionData.RawData.OriginalStateNamesByNumber{iTSe}(DecimalEvents(1:TrialEndInx(iT)-1)) 'EndTrial'];
    else        
...
for iTSe = 1:SessionData.nTrials
    ...
    TE_FD.CenterPortEntry(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 2,1,'last'))) ;
    
    if ~isempty(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 7)) % 'FreeChoice' exists, no early withdrawal
        
        TE_FD.CenterPortExit(iTSe) =  Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 7)) ;
        TE_FD.CenterPortRewardON(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 4));
        TE_FD.CenterPortRewardOFF(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 5));
        TE_FD.FrequencyON(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 5));
        TE_FD.FrequencyOFF(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 6,1))); % first exit
        
        if ~isempty(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 8)) % 'LeftRewardDelay' exists
            
            TE_FD.LeftPortEntry(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 8));
            
            if ~isempty(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 9)) % 'LeftReward' exists
                
                TE_FD.LeftPortExitFirst(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 16, 1, 'first')));
                TE_FD.LeftPortExitLast(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 16, 1, 'last')));
                TE_FD.Reward(iTSe) = 1;
                TE_FD.LeftRewardON(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 9));
                TE_FD.LeftRewardOFF(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 15, 1, 'first')));
                
            else % early withdrawal from reward port (leads to timeout)
                TE_FD.LeftPortExitFirst(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 17));
        ...
        elseif ~isempty(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 10)) % 'RightRewardDelay' exists
            ...
            TE_FD.RightPortEntry(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 10));
            
            if ~isempty(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 11)) % 'RightReward' exists
                
                TE_FD.RightPortExitFirst(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 16, 1, 'first')));
                TE_FD.RightPortExitLast(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 16, 1, 'last')));
                TE_FD.Reward(iTSe) = 1;
                TE_FD.RightRewardON(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 11));
                TE_FD.RightRewardOFF(iTSe) = Timestamps(TrialInxFD(find(TrialStatesByNumber_FD(TrialInxFD) == 15, 1, 'first')));
                
            else % early withdrawal from reward port (leads to timeout)
                TE_FD.RightPortExitFirst(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 17));
        ...
        elseif ~isempty(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 14)) % 'IncorrTrial' exists
            ...
            if TE_FD.TrialTypes(iTSe) == 1 % left would have been correct choice, but visited right
                ...
                TE_FD.RightPortEntry(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 14));
                TE_FD.RightPortExitFirst(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 0));
                TE_FD.RightPortExitLast(iTSe) = TE_FD.RightPortExitFirst(iTSe);
                
            else % right would have been correct choice, but visited left
                
                TE_FD.LeftPortEntry(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 14));
                TE_FD.LeftPortExitFirst(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 0));
                TE_FD.LeftPortExitLast(iTSe) = TE_FD.LeftPortExitFirst(iTSe);
            end
        ...
        TE_FD.CenterPortExit(iTSe) = Timestamps(TrialInxFD(TrialStatesByNumber_FD(TrialInxFD) == 17));
        TE_FD.NoReward(iTSe) = 1;
...
% extract states according to decimal events
TrialStates_FC = [];
 iT = iT + 1; % sometimes shift is needed by one step
for iTSe = 1:SessionData.nTrials
...
SessionShift = length(TrialStates_FD);

for iTSe = 1:SessionData.nTrials
    
    TrialInxFC = TrialStartInxFC(iTSe):TrialEndInxFC(iTSe);
    TimeInx = (TrialStartInxFC(iTSe):TrialEndInxFC(iTSe)) + SessionShift;
    TE_FC.CenterPortEntry(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 2,1,'last'))) ;
    
    if ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 7)) % 'FreeChoice' exists, no early withdrawal
        
        TE_FC.CenterPortExit(iTSe) =  Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 7,1))) ; % first instance of the state
        TE_FC.CenterPortRewardON(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 4,1))); % for some few trials there are 2 4 states
        TE_FC.CenterPortRewardOFF(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 6));
        
        if ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 8)) % 'LeftRewardDelay' exists
            
            TE_FC.LeftPortEntry(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 8));
            
            if ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 9)) % 'LeftReward' exists
                
                TE_FC.LeftPortExitFirst(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 16, 1, 'first')));
                TE_FC.LeftPortExitLast(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 16, 1, 'last')));
                TE_FC.Reward(iTSe) = 1;
                TE_FC.LeftRewardON(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 9));
                TE_FC.LeftRewardOFF(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 15, 1, 'first')));
                
            else % early withdrawal from reward port (leads to end of trial)
                TE_FC.LeftPortExitFirst(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 0));
        ...
        elseif ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 12)) % 'NoRewardLeft' exists
            ...
            TE_FC.LeftPortEntry(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 12));
            TE_FC.LeftPortExitFirst(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 0));
            TE_FC.LeftPortExitLast(iTSe) = TE_FC.LeftPortExitFirst(iTSe);
            TE_FC.NoReward(iTSe) = 1;
        ...
        elseif ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 30)) % 'LeftInactivated' exists
            ...
            TE_FC.LeftPortEntry(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 30));
            TE_FC.LeftPortExitFirst(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 0));
            TE_FC.LeftPortExitLast(iTSe) = TE_FC.LeftPortExitFirst(iTSe);
            TE_FC.NoReward(iTSe) = 1;
        ...
        elseif ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 10)) % 'RightRewardDelay' exists
            ...
            TE_FC.RightPortEntry(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 10));
            
            if ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 11)) % 'RightReward' exists
                
                TE_FC.RightPortExitFirst(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 16, 1, 'first')));
                TE_FC.RightPortExitLast(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 16, 1, 'last')));
                TE_FC.Reward(iTSe) = 1;
                TE_FC.RightRewardON(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 11));
                TE_FC.RightRewardOFF(iTSe) = Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 15, 1, 'first')));
                
            else % early withdrawal from reward port (leads to end of trial)
                TE_FC.RightPortExitFirst(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 0));
        ...
        elseif ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 13)) % 'NoRewardRight' exists
            ...
            TE_FC.RightPortEntry(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 13));
            TE_FC.RightPortExitFirst(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 0));
            TE_FC.RightPortExitLast(iTSe) = TE_FC.RightPortExitFirst(iTSe);
            TE_FC.NoReward(iTSe) = 1;
        ...
        elseif ~isempty(TrialInxFC(TrialStatesByNumber_FC(TrialInxFC) == 31)) % 'RightInactivated' exists
            ...
            TE_FC.RightPortEntry(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 31));
            TE_FC.RightPortExitFirst(iTSe) = Timestamps(TimeInx(TrialStatesByNumber_FC(TrialInxFC) == 0));
            TE_FC.RightPortExitLast(iTSe) = TE_FC.RightPortExitFirst(iTSe);
            TE_FC.NoReward(iTSe) = 1;
        ...
        TE_FC.CenterPortExit(iTSe) =  Timestamps(TimeInx(find(TrialStatesByNumber_FC(TrialInxFC) == 17,1,'first')));
        TE_FC.NoReward(iTSe) = 1;
