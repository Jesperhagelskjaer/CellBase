...
%                   TRIALS{iPART} = findpos(TE.(exp2eval),str2double(vals));
                    TRIALS{iPART} = findpos(TE.(exp2eval),str2num(vals)); %dk
%                     if size(TE.(exp2eval),2) > 1
%                     [row ~] = find(TE.(exp2eval) == str2num(vals)); %dk hack 
%                     TRIALS{iPART} = row; %dk hack 
%                     end
                    TAGS{iPART} = [exp2eval '=' newtag];
                    RTAGS{iPART} = [exp2eval '=' vals];
                else
%                   TRIALS{iPART} = findpos(TE.(condition),1,TE.(exp2eval),str2double(vals));
                    TRIALS{iPART} = findpos(TE.(condition),1,TE.(exp2eval),str2num(vals)); %dk
%                     if size(TE.(exp2eval),2) > 2
%                     [row ~] = find(TE.(exp2eval) == str2num(vals)); %dk hack 
%                     TRIALS{iPART} = row; %dk hack 
%                     end
                    TRIALS{iPART} = intersect(find(~isnan(TE.(condition))), TRIALS{iPART}); %dk
