function wave = extractSpikeWaveforms(cellid,system,SpikeTimes,varargin)
...
elseif system == 'OpenEphys'
TIMEFACTOR = getpref('cellbase','timefactor');    % scaling factor to convert spike times into seconds
Spikefile = cellid2fnames(cellid);
Spikefile = [Spikefile(1:end-6),'.spikes'];
[all_spikes all_waves] = LoadTT_openephys(Spikefile); 
SpikeTimes = round(SpikeTimes/TIMEFACTOR); % rounding the spiketimes
all_spikes = round(all_spikes/TIMEFACTOR);
% [all_waves all_spikes]  = load_open_ephys_data_faster(Spikefile); 
...
for k = 1:n
     ...
     shift = nSamples - minx;   % shift is decided based on the largest channel
     aligned_wv(k,mx,shift:shift+nSamples-2) = wv(k,mx,:);
     for oc = setdiff(1:4,mx)   % align other channels with the same shift
         aligned_wv(k,oc,shift:shift+nSamples-2) = wv(k,oc,:);
     end
end
aligned_wv = aligned_wv(:,:,32:32+nSamples);     % transform back to original size
