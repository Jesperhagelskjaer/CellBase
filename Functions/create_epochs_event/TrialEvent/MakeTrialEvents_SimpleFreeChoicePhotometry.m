...
if AquisitionSyst == 'OpenEphys'
    ...
    MaxNStates = 20;
    DecimalEvents(DecimalEvents>MaxNStates)= -1;
...
if nTrialEnds == nT_FC + 1
    ...
    inx1 = find(DecimalEvents == 1,1,'first');
    if inx1 > 1
        TrialStates_FC = cell(1,inx1-1);
        for i = 1:inx1-1
            TrialStates_FC{1,i} = ['Void'];
        end
    else
        TrialStates_FC = [];
    end
    ...
    for iTSe = 1:SessionData.nTrials
        ...
        if iTSe == 1
            TrialStates_FC = [TrialStates_FC SessionData.RawData.OriginalStateNamesByNumber{iTSe}(DecimalEvents(inx1:TrialEndInx(iTSe)-1)) 'EndTrial'];
        else
