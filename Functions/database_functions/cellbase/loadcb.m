...
if nargin == 0
    ...
    % fname has 0 in front of every TT channel lead in openephys format which needs to be added. Duda
%     fname = [fname(1:strfind(fname,'_')),'0',fname(strfind(fname,'_')+1:end)]; 
    
    TIMEFACTOR = getpref('cellbase','timefactor');    % scaling factor to convert spike times into seconds
%      TIMEFACTOR = 1.0000e-03; %dk
    if nargout == 1     % if we are loading into a variable
        if strncmpi(filetype,'Spikes',5)
            x = load(fname);
            x.tSpikes = double(x.tSpikes); %dk
%             varargout{1} = x.TS; %*TIMEFACTOR;  % if we are loading spikes, then multiply with conversion factor
            varargout{1} = x.tSpikes*TIMEFACTOR; % openephys format
