...
dendrogram(Z)
T = cluster(Z,'cutoff',80,'Criterion','distance');
...
for i = 1:numel(unique(T))
    if size(shiftMatrix(:,:,T == i),3) > 20 %more than X spikes in the hiarchi cluster
        ...
        for j = 1:size(shiftMatrix,2)
            subplot(1,size(shiftMatrix,2),j)
            hold on
...
figure
for i = 1:numel(unique(c)) %number of clusters in the TT files
    idx   = c == i; 
    test  = T(logical(c == i));   %find the comparison between Hiearchi-clustering 
    value = tabulate(test);       %tabulate the value that the TT files compares to the Hiearchi-clustering
    tst   = find(value(:,3) > 5);   %more than percent X spikes 
    for m = 1:size(shiftMatrix,2) %number of chs
