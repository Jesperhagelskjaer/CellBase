...
%add_analysis(@template_real_time,1,'property_names',{'confusion'},'arglist',{});
%add_analysis(@template_real_time,0,'property_names',{'confusion'},'arglist',{'cells',[28]});
...
if (cellid == 0)
    ...
    addParameter(prs,'xAxis_on',[-2 10])  % online
    addParameter(prs,'shift',-15)         % online time (range should be as small as possible)
    addParameter(prs,'xAxis',[-5 5])      % online time (range should be as small as possible)
    ...
    addParameter(prs,'spline',1)       
    addParameter(prs,'purity',1)          %look at the contamination for each cluster on each on
    parse(prs,varargin{:})
