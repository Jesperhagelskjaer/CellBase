function [DM LrC valid_channels] = LRatio2(cellid,system,varargin)
...
if isempty(g.valid_channels)
    valid_channels = check_channel_validity(cellid,system);
else
...
propfn = [getpref('cellbase','cell_pattern') num2str(t) '.spikes'];
propfn_full = [propfn_path filesep propfn];
all_spikes = LoadTT_openephys(propfn_full); 
[wv allspikes] = load_open_ephys_data_faster(propfn_full);  
end
...
% this somehow works only at the moment for neuralynx data only
if system == 'Neuralynx'
...
for k = 1:length(g.feature_names)
    ...
    if system == 'OpenEphys' %dk
    propfn = [basename '_feature_' g.feature_names{k}];   % name of feature file (e.g. TT1_Amplitude)
    elseif system == 'Neuralynx'
...
XC = X(:,inx);
D = mahal(X',XC');
DC = D(cinx(1:end-1)); % hack was end -1, 
sDC = sort(DC);
linx = length(inx);
if linx <= length(sDC)
    DM = sDC(length(inx));
